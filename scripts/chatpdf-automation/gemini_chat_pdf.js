 ...
 */
async function waitForGeminiResponseFinished(page, timeoutMs = 300000, quietMs = 6000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (Date.now() - lastProbeLogAt >= 5000) {
      ...
      console.log(`Gemini 回答检测: hasStop=${hasStop} stopSeen=${stopSeen} textLength=${textLength} quietForMs=${quietForMs}`);
    }
...
async function chatWithGemini(pdfPath, chatName, existingPage, outputFile = null) {
  ...
  try {
    ...
    try {
      ...
      const sessionMenu = page.locator('[data-test-id="conversation-actions-menu-icon-button"]');
